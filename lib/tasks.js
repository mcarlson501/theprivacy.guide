...
/**
 * Get the total number of tasks (lightweight, no markdown processing)
 * @returns {number} Number of task files
 */
export function getTaskCount() {
  try {
    if (!fs.existsSync(tasksDirectory)) return 0;
    return fs.readdirSync(tasksDirectory).filter(name => name.endsWith('.md')).length;
  } catch (error) {
