        ...
        {filteredArticles.length > 0 ? (
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {filteredArticles.map((article) => (
              <article
                key={article.id}
                className="bg-white dark:bg-dark-surface rounded-xl border border-gray-200 dark:border-dark-border p-6 hover:shadow-lg dark:hover:shadow-xl transition-shadow"
              >
                {/* Category & Date */}
                ...
                {/* Title */}
                <h2 className="text-xl font-bold text-charcoal-gray dark:text-dark-text mb-3 line-clamp-2">
                  {article.title}
                </h2>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {article.relatedTasks.slice(0, 2).map((taskId) => (
                        <Link
                          key={taskId}
                          href={`/tasks/${taskId}`}
                          className="text-xs bg-friendly-blue/10 text-friendly-blue hover:bg-friendly-blue/20 px-2 py-1 rounded transition-colors"
                        >
                          {taskId.replace(/-/g, ' ')}
                        </Link>
                      ))}
                ...
                <Link
                  ...
                  className="inline-flex items-center text-friendly-blue hover:text-blue-600 font-medium text-sm transition-colors"
                >
                  Read Full Article
